...
# ===================== Attention Layer + Residual Connection (Core Opt: Temporal Feature Extraction) =====================
class TimeSeriesAttention(nn.Module):
    """
    Temporal Attention Layer: Assign weights to each time-step feature of BiLSTM output, focus on key time steps
    Input: [batch, seq_len, hidden_dim]  Output: [batch, seq_len, hidden_dim]
    """
    ...
    def forward(self, x):
        ...
        attn_weights = F.softmax(score, dim=1)  # Normalize on time-step dimension
        x_attn = x * attn_weights  # Weighted features
        return x_attn, attn_weights

class ResidualBlock(nn.Module):
    """
    Residual Block: Alleviate gradient vanishing in deep BiLSTM, improve feature reuse
    """
...
# ===================== CNN+BiLSTM Model Construction (Core Opt: CNN提取局部特征 + BiLSTM捕捉时序依赖) =====================
class CNNBILSTMPowerModel(nn.Module):
    def __init__(self, input_dim, cnn_filters=64, cnn_kernel_size=3, lstm_hidden_dim=128, lstm_num_layers=2, dropout_rate=0.4):
        super(CNNBILSTMPowerModel, self).__init__()
        self.input_norm = nn.LayerNorm(input_dim)
        
        # CNN层：提取时间序列的局部特征（卷积核大小3，覆盖相邻3个时间步）
        self.cnn1 = nn.Conv1d(in_channels=input_dim, out_channels=cnn_filters, kernel_size=cnn_kernel_size, padding='same')
        self.cnn2 = nn.Conv1d(in_channels=cnn_filters, out_channels=cnn_filters*2, kernel_size=cnn_kernel_size, padding='same')
        self.cnn_norm = nn.BatchNorm1d(cnn_filters*2)
        self.cnn_activation = nn.GELU()
        self.cnn_dropout = nn.Dropout(dropout_rate)
        
        # BiLSTM层：捕捉时序依赖（双向LSTM，和原模型完全一致）
        self.bilstm = nn.LSTM(
            cnn_filters*2, lstm_hidden_dim, num_layers=lstm_num_layers, 
            bidirectional=True, batch_first=True, 
            dropout=dropout_rate if lstm_num_layers > 1 else 0.0
        )
        self.hidden_dim = lstm_hidden_dim * 2  # 双向拼接维度
        
        # 时间注意力层：聚焦关键时间步特征
        self.attn = TimeSeriesAttention(self.hidden_dim)
    ...
    def forward(self, x):
        ...
        # CNN层需要将特征维度放到通道维度：[batch, input_dim, seq_len]
        ...
        # 转换回BiLSTM输入格式：[batch, seq_len, cnn_filters*2]
        x_bilstm = x_cnn.permute(0, 2, 1)
        
        # BiLSTM层
        bilstm_out, _ = self.bilstm(x_bilstm)  # [batch, seq_len, 2*hidden_dim]
        
        # 注意力层
        attn_out, _ = self.attn(bilstm_out)
        
        # 残差块和输出
        x = self.res1(attn_out)
        x = self.dropout(x)
        x = self.res2(x)
        return self.fc(x)  # [batch, seq_len, 1]

def build_model(input_dim):
    """Build CNN+BiLSTM model and print parameter count"""
    model = CNNBILSTMPowerModel(
        input_dim=input_dim,
        cnn_filters=64,
        cnn_kernel_size=3,
        lstm_hidden_dim=128,
    ...
    print(f"【Model Construction】CNN+BiLSTM Model | Total parameters: {total_params/1000:.1f}k, Trainable parameters: {trainable_params/1000:.1f}k")
    return model
...
# ===================== Training Function (Core Opt: Loss Function + Training Strategy + Gradient Clipping) =====================
def train_model(tensor_dict, input_dim, power_scaler, resume_path=None):
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        if val_rmse < best_val_rmse:
            ...
            torch.save(model.state_dict(), "best_cnn_bilstm_power_model_paper.pth")
        ...
        if (epoch + 1) % 5 == 0:
            ...
            torch.save(checkpoint, f"cnn_bilstm_checkpoint_epoch_{epoch+1}_paper.pth")
            print(f"✅ Auto-save checkpoint: cnn_bilstm_checkpoint_epoch_{epoch+1}_paper.pth (Current Best MAE: {best_val_mae:.4f})")
    ...
    if train_losses and val_losses:
        ...
        plt.title("CNN+BiLSTM Training and Validation Loss Curves (MAE Dominated)", fontsize=14, pad=10)
        plt.legend(fontsize=10)
        plt.grid(alpha=0.3, linestyle='--')
        plt.tight_layout()
        plt.savefig("cnn_bilstm_loss_curve.png", dpi=300, bbox_inches='tight')
        plt.close()
        print(f"\n【Curve Saved】CNN+BiLSTM Training/Validation loss curve saved as: cnn_bilstm_loss_curve.png")

    # Load best model
    if os.path.exists("best_cnn_bilstm_power_model_paper.pth"):
        model.load_state_dict(torch.load("best_cnn_bilstm_power_model_paper.pth", map_location=DEVICE))
        print(f"\n【Model Loaded】✅ Load best CNN+BiLSTM model, Best Val RMSE: {best_val_rmse:.4f} | MAE: {best_val_mae:.4f}")
    else:
        print("【Warning】No best model saved, using the last trained model!")
    return model

# ===================== True vs Predicted Power Visualization Function =====================
def plot_pred_true(y_true, y_pred, rmse, mae, data_name="Test Set", save_path="pred_vs_true.png"):
    ...
    plt.plot(x_ticks, y_pred_plot, color="#E63946", linewidth=1.2, label=f"Predicted Power (CNN+BiLSTM)", alpha=0.8, linestyle='--')
    ...
    plt.title(f"{data_name}: True Power vs CNN+BiLSTM Predicted Power", fontsize=16, pad=20, fontweight='bold')
...
# ===================== Evaluation Function (Core Opt: Fine Post-processing + Visualization) =====================
def evaluate_model(model, X_data, y_data, power_scaler, data_name="Test Set"):
    ...
    plot_pred_true(y_true, pred, rmse, mae, data_name, save_path=f"cnn_bilstm_{data_name.lower().replace(' ', '_')}_pred_vs_true.png")

    # Print detailed evaluation results
    print(f"\n===================== {data_name} Evaluation Results (CNN+BiLSTM) =====================")
...
def main(csv_file_path, resume_checkpoint=None):
    try:
        ...
        # 2. Model Training
        print("\n" + "="*60 + f"\nStart CNN+BiLSTM Training ({'Resume' if resume_checkpoint else 'Initial'})\n" + "="*60)
        best_model = train_model(tensor_dict, input_dim, power_scaler, resume_path=resume_checkpoint)
        
        # 3. Test Set Evaluation
        print("\n" + "="*60 + "\nStart Test Set Evaluation (CNN+BiLSTM)\n" + "="*60)
        ...
        # print("\n" + "="*60 + "\nStart Train Set Evaluation (CNN+BiLSTM)\n" + "="*60)
        ...
        print(f"\n【Final CNN+BiLSTM Training Result Summary】")
...
if __name__ == "__main__":
    ...
    # best_model, test_rmse, test_mae = main(CSV_FILE_PATH, resume_checkpoint="cnn_bilstm_checkpoint_epoch_50_paper.pth")