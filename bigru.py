...
# ===================== Residual Connection (保留原优化，无Attention) =====================
class ResidualBlock(nn.Module):
    """
    Residual Block: Alleviate gradient vanishing in deep GRU, improve feature reuse
    """
...
# ===================== BiGRU模型（核心修改：双向GRU替换双向LSTM，无Attention） =====================
class BiGRUPowerModel(nn.Module):
    def __init__(self, input_dim, hidden_dim=128, num_layers=2, dropout_rate=0.4):
        super(BiGRUPowerModel, self).__init__()
        self.input_norm = nn.LayerNorm(input_dim)
        
        # 核心修改：双向GRU替换双向LSTM，其余配置和原BiLSTM完全一致
        self.gru = nn.GRU(
            input_dim, hidden_dim, num_layers=num_layers, 
            bidirectional=True, batch_first=True,  # 保留双向配置，和原BiLSTM对齐
            dropout=dropout_rate if num_layers > 1 else 0.0
        )
        self.hidden_dim = hidden_dim * 2  # 双向输出，维度×2，和原BiLSTM对齐
    ...
    def forward(self, x):
        ...
        gru_out, _ = self.gru(x)  # [batch, seq_len, 2*hidden_dim] 双向GRU输出，无注意力
        x = self.res1(gru_out)     # 直接传入残差块
        x = self.dropout(x)
        x = self.res2(x)
        return self.fc(x)  # [batch, seq_len, 1] 输出维度和所有模型完全一致

def build_model(input_dim):
    """Build BiGRU模型 and print parameter count"""
    model = BiGRUPowerModel(input_dim, hidden_dim=128, num_layers=2, dropout_rate=0.4).to(DEVICE)
    total_params = sum(p.numel() for p in model.parameters())
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"【Model Construction】BiGRU模型 | Total parameters: {total_params/1000:.1f}k, Trainable parameters: {trainable_params/1000:.1f}k")
    return model
...
# ===================== Training Function (完全复用原逻辑，仅修改模型文件命名) =====================
def train_model(tensor_dict, input_dim, power_scaler, resume_path=None):
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        if val_rmse < best_val_rmse:
            ...
            torch.save(model.state_dict(), "best_power_model_BiGRU.pth")  # BiGRU专属模型文件名
        ...
        if (epoch + 1) % 5 == 0:
            ...
            torch.save(checkpoint, f"checkpoint_BiGRU_epoch_{epoch+1}_paper.pth")  # BiGRU专属检查点
            print(f"✅ Auto-save checkpoint: checkpoint_BiGRU_epoch_{epoch+1}_paper.pth (Current Best MAE: {best_val_mae:.4f})")
    ...
    if train_losses and val_losses:
        ...
        plt.title("Training and Validation Loss Curves (BiGRU | MAE Dominated)", fontsize=14, pad=10)  # BiGRU专属标题
        plt.legend(fontsize=10)
        plt.grid(alpha=0.3, linestyle='--')
        plt.tight_layout()
        plt.savefig("loss_curve_BiGRU.png", dpi=300, bbox_inches='tight')  # BiGRU专属损失曲线
        plt.close()
        print(f"\n【Curve Saved】Training/Validation loss curve saved as: loss_curve_BiGRU.png")

    # Load best model
    if os.path.exists("best_power_model_BiGRU.pth"):
        model.load_state_dict(torch.load("best_power_model_BiGRU.pth", map_location=DEVICE))
        print(f"\n【Model Loaded】✅ Load best BiGRU model, Best Val RMSE: {best_val_rmse:.4f} | MAE: {best_val_mae:.4f}")
    else:
        print("【Warning】No best model saved, using the last trained model!")
    return model

# ===================== True vs Predicted Power Visualization Function (复用+专属命名) =====================
def plot_pred_true(y_true, y_pred, rmse, mae, data_name="Test Set", save_path="pred_vs_true.png"):
    ...
    plt.title(f"{data_name}: True Power vs Predicted Power (BiGRU)", fontsize=16, pad=20, fontweight='bold')  # BiGRU专属标题
...
# ===================== Evaluation Function (复用+专属命名/标题) =====================
def evaluate_model(model, X_data, y_data, power_scaler, data_name="Test Set"):
    ...
    # Call visualization function (Times New Roman font, BiGRU专属命名)
    plot_pred_true(y_true, pred, rmse, mae, data_name, save_path=f"BiGRU_{data_name.lower().replace(' ', '_')}_pred_vs_true.png")

    # Print detailed evaluation results (BiGRU专属标题)
    print(f"\n===================== {data_name} Evaluation Results (BiGRU) =====================")
...
def main(csv_file_path, resume_checkpoint=None):
    try:
        ...
        # 2. Model Training
        print("\n" + "="*60 + f"\nStart Training (BiGRU | {'Resume' if resume_checkpoint else 'Initial'})\n" + "="*60)
        best_model = train_model(tensor_dict, input_dim, power_scaler, resume_path=resume_checkpoint)
        
        # 3. Test Set Evaluation
        print("\n" + "="*60 + "\nStart Test Set Evaluation (BiGRU)\n" + "="*60)
...
if __name__ == "__main__":
    ...
    # Resume Training (uncomment and specify BiGRU checkpoint file)
    # best_model, test_rmse, test_mae = main(CSV_FILE_PATH, resume_checkpoint="checkpoint_BiGRU_epoch_50_paper.pth")