...
class TimeSeriesAttention(nn.Module):
    """
    Temporal Attention Layer: Assign weights to each time-step feature of LSTM output, focus on key time steps
...
# ===================== Model Construction (Core Opt: 2-layer BiLSTM + Attention + Residual + Effective Dropout) =====================
class LSTMPowerModel(nn.Module):
    def __init__(self, input_dim, hidden_dim=128, num_layers=2, dropout_rate=0.4):
        ...
        # 2-layer bidirectional LSTM with effective Dropout (num_layers > 1)
        self.lstm = nn.LSTM(
            input_dim, hidden_dim, num_layers=num_layers, 
            bidirectional=True, batch_first=True, 
            dropout=dropout_rate if num_layers > 1 else 0.0
        )
        self.hidden_dim = hidden_dim * 2  # Bidirectional concatenation dimension
        
        # Temporal attention layer to focus on key time-step features
        self.attn = TimeSeriesAttention(self.hidden_dim)
        
        # Residual blocks for feature reuse and training stability
        self.res1 = ResidualBlock(self.hidden_dim, self.hidden_dim)
        self.res2 = ResidualBlock(self.hidden_dim, self.hidden_dim // 2)
        
        # Adjusted Dropout rate (0.4) to reduce overfitting
    ...
    def forward(self, x):
        ...
        attn_out, _ = self.attn(lstm_out)  # Temporal attention weighting
...
def build_model(input_dim):
    """Build optimized model and print parameter count"""
    model = LSTMPowerModel(input_dim, hidden_dim=128, num_layers=2, dropout_rate=0.4).to(DEVICE)
    total_params = sum(p.numel() for p in model.parameters())
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"【Model Construction】Total parameters: {total_params/1000:.1f}k, Trainable parameters: {trainable_params/1000:.1f}k")
    return model
...
def train_model(tensor_dict, input_dim, power_scaler, resume_path=None):
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        if val_rmse < best_val_rmse:
            ...
            torch.save(model.state_dict(), "best_power_model_paper.pth")
        ...
        if (epoch + 1) % 5 == 0:
            ...
            torch.save(checkpoint, f"checkpoint_epoch_{epoch+1}_paper.pth")
            print(f"✅ Auto-save checkpoint: checkpoint_epoch_{epoch+1}_paper.pth (Current Best MAE: {best_val_mae:.4f})")
    ...
    if train_losses and val_losses:
        ...
        plt.title("Training and Validation Loss Curves (MAE Dominated)", fontsize=14, pad=10)
        plt.legend(fontsize=10)
        plt.grid(alpha=0.3, linestyle='--')
        plt.tight_layout()
        plt.savefig("loss_curve.png", dpi=300, bbox_inches='tight')
        plt.close()
        print(f"\n【Curve Saved】Training/Validation loss curve saved as: loss_curve.png")

    # Load best model
    if os.path.exists("best_power_model_paper.pth"):
        model.load_state_dict(torch.load("best_power_model_paper.pth", map_location=DEVICE))
        print(f"\n【Model Loaded】✅ Load best model, Best Val RMSE: {best_val_rmse:.4f} | MAE: {best_val_mae:.4f}")
    else:
...
def plot_pred_true(y_true, y_pred, rmse, mae, data_name="Test Set", save_path="pred_vs_true.png"):
    ...
    plt.title(f"{data_name}: True Power vs Predicted Power", fontsize=16, pad=20, fontweight='bold')
...
def evaluate_model(model, X_data, y_data, power_scaler, data_name="Test Set"):
    ...
    plot_pred_true(y_true, pred, rmse, mae, data_name, save_path=f"{data_name.lower().replace(' ', '_')}_pred_vs_true.png")

    # Print detailed evaluation results
    print(f"\n===================== {data_name} Evaluation Results =====================")
...
def main(csv_file_path, resume_checkpoint=None):
    try:
        ...
        # 2. Model Training
        print("\n" + "="*60 + f"\nStart Training ({'Resume' if resume_checkpoint else 'Initial'})\n" + "="*60)
...
if __name__ == "__main__":
    ...
    # best_model, test_rmse, test_mae = main(CSV_FILE_PATH, resume_checkpoint="checkpoint_epoch_50_paper.pth")