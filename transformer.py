...
# ===================== Transformer模型核心（时间序列专用，仅Encoder）=====================
class PositionalEncoding(nn.Module):
    """
    可学习位置编码：适配时间序列，比固定正弦位置编码更灵活，学习时序位置特征
    ...
    def forward(self, x):
        ...
        seq_len = x.shape[1]
        pos_emb = self.pos_emb(self.pos_idx[:, :seq_len])  # [1, seq_len, d_model]
        return x + pos_emb  # 位置编码与特征相加

class TransformerPowerModel(nn.Module):
    """
    时间序列专用Transformer模型（仅Encoder）：适配无人机功率预测的回归任务
    核心设计：1. 特征投影层匹配d_model 2. 可学习位置编码 3. 堆叠Encoder 4. 层归一化+Dropout防过拟合
    """
    def __init__(self, input_dim, d_model=128, nhead=8, num_layers=2, dropout_rate=0.4):
        super().__init__()
        self.input_dim = input_dim
        self.d_model = d_model
        
        # 输入特征投影：将原始特征维度映射到Transformer的d_model维度
        self.feat_proj = nn.Linear(input_dim, d_model)
        self.input_norm = nn.LayerNorm(d_model)  # 输入层归一化
        
        # 位置编码（时间序列核心，学习时序位置信息）
        self.pos_enc = PositionalEncoding(d_model)
        
        # Transformer Encoder层配置
        encoder_layers = nn.TransformerEncoderLayer(
            d_model=d_model,
            nhead=nhead,
            dim_feedforward=d_model * 4,  # 前馈层维度为d_model*4（行业标准）
            dropout=dropout_rate,
            activation='gelu',  # 与原模型一致，用GELU减少梯度消失
            batch_first=True,  # 适配[batch, seq_len, d_model]输入格式
        ...
        self.transformer_encoder = nn.TransformerEncoder(
            ...
            norm=nn.LayerNorm(d_model)  # 最终层归一化
        )
        
        # Dropout防过拟合（与原模型保持一致的dropout_rate）
        self.dropout = nn.Dropout(dropout_rate)
        
        # 输出层：将d_model维度映射到1（功率预测值）
        self.fc_out = nn.Linear(d_model, 1)

    def forward(self, x):
        # x.shape: [batch, seq_len, input_dim]
        # 1. 特征投影+归一化
        x = self.feat_proj(x)  # [batch, seq_len, d_model]
        x = self.input_norm(x)
        # 2. 位置编码
        x = self.pos_enc(x)
        # 3. Transformer Encoder特征提取
        x = self.transformer_encoder(x)  # [batch, seq_len, d_model]
        # 4. Dropout+输出预测
        x = self.dropout(x)
        out = self.fc_out(x)  # [batch, seq_len, 1]
        return out

def build_model(input_dim):
    """Build Transformer模型并打印参数量，与原LSTM模型参数量级匹配"""
    model = TransformerPowerModel(
        input_dim=input_dim,
        d_model=128,  # 与原LSTM hidden_dim一致
        nhead=8,      # 128/8=16，每个头维度16（合理范围）
        num_layers=2, # 与原LSTM层数一致
        dropout_rate=0.4  # 与原模型dropout一致
    ).to(DEVICE)
    # 计算参数量
    total_params = sum(p.numel() for p in model.parameters())
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"【Transformer Model Construction】Total parameters: {total_params/1000:.1f}k, Trainable parameters: {trainable_params/1000:.1f}k")
    return model
...
def train_model(tensor_dict, input_dim, power_scaler, resume_path=None):
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        if val_rmse < best_val_rmse:
            ...
            torch.save(model.state_dict(), "best_power_model_transformer.pth")  # 重命名模型文件，区分Transformer
        ...
        if (epoch + 1) % 5 == 0:
            ...
            torch.save(checkpoint, f"checkpoint_epoch_{epoch+1}_transformer.pth")  # 重命名检查点，区分Transformer
            print(f"✅ Auto-save checkpoint: checkpoint_epoch_{epoch+1}_transformer.pth (Current Best MAE: {best_val_mae:.4f})")
    ...
    if train_losses and val_losses:
        ...
        plt.title("Transformer Training and Validation Loss Curves (MAE Dominated)", fontsize=14, pad=10)  # 标题修改为Transformer
        plt.legend(fontsize=10)
        plt.grid(alpha=0.3, linestyle='--')
        plt.tight_layout()
        plt.savefig("loss_curve_transformer.png", dpi=300, bbox_inches='tight')  # 损失曲线重命名
        plt.close()
        print(f"\n【Curve Saved】Training/Validation loss curve saved as: loss_curve_transformer.png")

    # Load best model
    if os.path.exists("best_power_model_transformer.pth"):
        model.load_state_dict(torch.load("best_power_model_transformer.pth", map_location=DEVICE))
        print(f"\n【Model Loaded】✅ Load best Transformer model, Best Val RMSE: {best_val_rmse:.4f} | MAE: {best_val_mae:.4f}")
    else:
...
def plot_pred_true(y_true, y_pred, rmse, mae, data_name="Test Set", save_path="pred_vs_true.png"):
    ...
    plt.title(f"{data_name}: True Power vs Predicted Power (Transformer)", fontsize=16, pad=20, fontweight='bold')  # 标题添加Transformer
...
def evaluate_model(model, X_data, y_data, power_scaler, data_name="Test Set"):
    ...
    # 可视化文件重命名，添加transformer标识
    save_path = f"{data_name.lower().replace(' ', '_')}_pred_vs_true_transformer.png"
    # Call visualization function (Times New Roman font)
    plot_pred_true(y_true, pred, rmse, mae, data_name, save_path=save_path)

    # Print detailed evaluation results
    print(f"\n===================== {data_name} Evaluation Results (Transformer) =====================")  # 标题添加Transformer
...
def main(csv_file_path, resume_checkpoint=None):
    try:
        ...
        # 2. Model Training
        print("\n" + "="*60 + f"\nStart Transformer Training ({'Resume' if resume_checkpoint else 'Initial'})\n" + "="*60)
        best_model = train_model(tensor_dict, input_dim, power_scaler, resume_path=resume_checkpoint)
        
        # 3. Test Set Evaluation
        print("\n" + "="*60 + "\nStart Test Set Evaluation (Transformer)\n" + "="*60)
        ...
        # print("\n" + "="*60 + "\nStart Train Set Evaluation (Transformer)\n" + "="*60)
        ...
        print(f"\n【Final Transformer Training Result Summary】")
...
if __name__ == "__main__":
    ...
    # best_model, test_rmse, test_mae = main(CSV_FILE_PATH, resume_checkpoint="checkpoint_epoch_50_transformer.pth")